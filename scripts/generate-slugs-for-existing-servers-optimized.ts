...
async function processProfile(profileUuid: string) {
  console.log(`Processing profile: ${profileUuid}`);
  ...
  try {
    ...
    if (serversWithoutSlugs.length === 0) {
      console.log(`No servers without slugs in profile ${profileUuid}`);
      return { profileUuid, processed: 0, errors: 0 };
    }
    
    console.log(`Found ${serversWithoutSlugs.length} servers without slugs in profile ${profileUuid}`);
    ...
    for (let i = 0; i < updates.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.log(`Updated ${batch.length} servers in profile ${profileUuid}`);
      } catch (error) {
        console.error(`Error updating batch in profile ${profileUuid}:`, error);
        errors += batch.length;
      }
    }
    ...
    console.error(`Error processing profile ${profileUuid}:`, error);
...
async function processProfilesInBatches(profileUuids: string[]) {
  ...
  for (let i = 0; i < profileUuids.length; i += PARALLEL_PROFILES) {
    ...
    console.log(`Completed ${Math.min(i + PARALLEL_PROFILES, profileUuids.length)} of ${profileUuids.length} profiles`);
  }
...
async function main() {
  ...
  try {
    ...
    console.log(`Found ${profileUuids.length} profiles with servers missing slugs`);
    ...
    console.log('\n=== Migration Complete ===');
    console.log(`Total profiles processed: ${profileUuids.length}`);
    console.log(`Total servers updated: ${totalProcessed}`);
